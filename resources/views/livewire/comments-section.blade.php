...
<div class="flex flex-col h-auto lg:h-full">
            ...
            <div class="bg-white border border-gray-200 rounded-[20px] p-4 mb-3 shadow-sm">
                ...
                <div>
                    ...
                    <button class="btn btn-sm btn-outline-secondary ms-2"
                        wire:click="setReplyTo({{ $comentario->id }}, '{{ addslashes($comentario->user->name) }}')">
                        Responder
                    </button>
                    @endif
                </div>
    ...
    <div class="flex-shrink-0 px-4 py-4 bg-white border-t border-gray-100 rounded-b-2xl">
        ...
        <!-- resources/views/components/comment-input.blade.php -->
<div>
  <!-- Indicador cuando se está respondiendo -->
  @if($replyToId)
  <div class="mb-2 d-flex align-items-center">
    <small class="text-muted">Respondiendo a <strong class="ms-1">{{ $replyToUser }}</strong></small>
    <button type="button" class="btn btn-sm btn-link text-danger ms-auto" wire:click="clearReply">Cancelar</button>
  </div>
  @endif

  <form wire:submit.prevent="store" class="d-flex gap-2 align-items-start">
    <!-- Campo oculto para parent_id -->
    <input type="hidden" wire:model="replyToId">

    <input
      type="text"
      wire:model.defer="comentario"
      class="flex-1 form-control form-control-sm {{ $errors->has('comentario') ? 'is-invalid' : '' }}"
      placeholder="{{ $replyToId ? 'Respondiendo a ' . $replyToUser : 'Agrega un comentario...' }}"
      maxlength="500"
      autocomplete="off"
