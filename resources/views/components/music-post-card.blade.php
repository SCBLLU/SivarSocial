...
<div class="bg-white rounded-2xl shadow-lg w-full max-w-md flex flex-col overflow-hidden"
    ...
    <div class="flex items-center w-full px-4 py-3 border-b border-gray-200">
        <a href="{{ route('posts.index', $user->username) }}" class="flex items-center">
            @if($user->imagen)
                <img src="{{ asset('perfiles') . '/' . $user->imagen }}" alt="Imagen de perfil de {{ $user->username }}"
        ...
        <div class="ml-3 flex-1">
            <a href="{{ route('posts.index', $user->username) }}"
                class="font-semibold text-gray-900 hover:underline">{{ $user->name }}</a>
    ...
    <div class="w-full flex justify-center"
        style="background: linear-gradient(135deg, {{ $post->dominant_color ?? '#1DB954' }}20 0%, {{ $post->dominant_color ?? '#1DB954' }}40 100%);">
        <div class="relative w-full max-w-sm p-6">
            <!-- Portada del álbum -->
            <div class="relative mb-4">
                <img src="{{ $post->spotify_album_image }}" alt="{{ $post->spotify_album_name }}"
                ...
                @if($post->spotify_preview_url)
                    <button onclick="toggleMusicPreview('{{ $post->spotify_preview_url }}', '{{ $post->id }}', this)"
                        ...
                        <div
                            ...
                            <svg class="w-8 h-8 text-gray-800 play-icon-{{ $post->id }}" fill="currentColor"
                                viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z"
                                    clip-rule="evenodd"></path>
                            </svg>
                            <svg class="w-8 h-8 text-gray-800 pause-icon-{{ $post->id }} hidden" fill="currentColor"
                                viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
    ...
    <div class="w-full px-4 py-3">
        <!-- Título y descripción con lógica especial -->
        <div class="mb-3">
            @if($post->titulo)
                <!-- Si tiene título, mostrar título -->
            ...
            @elseif($post->descripcion)
                <!-- Si NO tiene título pero SÍ descripción, descripción va como título -->
                <h4 class="text-gray-600 text-sm">{{ $post->descripcion }}</h4>
            @endif
        </div>
