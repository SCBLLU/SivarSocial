...
<div id="social-links-unified" class="space-y-4">
    ...
    <div class="flex justify-between items-center">
        ...
        @if($canManage && $socialLinks->count() < 4)
        ...
        @elseif($canManage && $socialLinks->count() >= 4)
    ...
    @if($canManage)
        <div id="addLinkForm" class="hidden bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
    ...
    @if($socialLinks->count() > 0)
        <div class="grid grid-cols-2 gap-2" id="socialLinksGrid">
            @foreach($socialLinks as $index => $link)
                <div class="social-link-item relative group overflow-hidden bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-lg p-2 border border-gray-100 dark:border-gray-700 transition duration-200 ease-in-out hover:shadow-md hover:-translate-y-0.5 hover:bg-gray-100 dark:hover:bg-gray-800"
                    ...
                    <div class="relative flex items-center space-x-2">
                        ...
                        <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 shadow"
                            ...
                            <i class="{{ $link->getPlatformIcon() }} text-base"
                        ...
                        @if($canManage)
                            <div class="flex items-center space-x-1 social-link-actions opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                {{-- Mover arriba --}}
                                @if(!$loop->first)
                                <button type="button" onclick="moveLink({{ $link->id }}, 'up')"
                                        class="p-1 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-all duration-200"
                                        title="Mover arriba">
                                ...
                                <button type="button" onclick="deleteLink({{ $link->id }})"
                                        class="p-1 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-all duration-200"
